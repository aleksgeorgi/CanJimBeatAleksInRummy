...
from app.models import db, RawScores
...
# def setup_datbase(): # TODO migrate using Alembic
#     try:
...
def add_data(df): #TODO REFACTOR USING SQLALCHEMY
    ...
    try:
        ...
        if 'game_number' in df.columns:
            df = df.drop(columns=['game_number'])
            
        # convert Dataframe rows to ORM objects 
        rows = [
            RawScores(
                jim_score=row['jim_score'],
                aleks_score=row['aleks_score']
            )
